...
class antropometricosManager(models.Manager):
	...
	def getByIdJoin(self,id):
		query='SELECT data.*,resultado.metabolismobasal,resultado.indiceadiposidad,resultado.obesidad FROM account_datosantropometricos as data INNER JOIN account_antropometricosresultado as resultado ON data.id=resultado.datosantropometricos_id where data.user_id='+str(id)+';'
